...
module.exports = function (config) {
  ...
  return {
    ...
    getUserAccountByUuid: function (userUuid, accountUuid) {
      return reqInstance('GET', '/users/' + userUuid + '/accounts/' + accountUuid)
    },
    ...
    getRate: function (queryParams) {
      return reqInstance('GET', resolveUrlWithParams('/rates', queryParams));
    },
...
function resolveUrlWithParams(url, queryParams) {
  // take in queryParams as object, and transform into ?key=value&key2=value2
  const stringifiedParams = Object.entries(queryParams).reduce((acc, [key, value], i, arr) => {
